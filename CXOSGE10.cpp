   ...
   else if (CaseSegment::instance()->getTSTAMP_LOCAL().length() >= 8)
   {
      ...
      for (;;)
      {
         ...
         if (!IF::Extract::instance()->getRecord("DSPEC   GE10    MULTICHB~Y", strValue))
            hQuery.setBasicPredicate(strLocator.c_str(), "FIN_TYPE", "NOT LIKE", "__1");
         if (CaseManifestSegment::instance()->getFile() == "DNRDM5" && CaseSegment::instance()->getTSTAMP_LOCAL().length() >= 14)
         {
            hQuery.setBasicPredicate(strLocator.c_str(), "TSTAMP_LOCAL", "=", CaseSegment::instance()->getTSTAMP_LOCAL().c_str());
         }
         else
         {
